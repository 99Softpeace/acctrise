...
export function WalletPage() {
  ...
  async function fundWallet() {
    ...
    try {
      const response = await fetch("/api/wallet/fund", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ amount: value, email: session.data?.user?.email, paymentMethod: "checkout", paymentGateway: "pocketfi", callbackUrl: `${window.location.origin}/dashboard/wallet?payment=processing` }) });
      const body = await response.json();
  ...
  return (
    <div className="mx-auto grid max-w-7xl gap-6">
      <PageHeader eyebrow="Wallet" title="Wallet funding" description="Fund your wallet by bank transfer or online payment. Your balance updates after payment confirmation." />
      <DedicatedAccountCard />
      <section className="grid gap-6 xl:grid-cols-[0.85fr_1.15fr]">
        <Surface className="p-5"><div className="rounded-lg bg-blue-700 p-5 text-blue-50"><p className="text-sm font-semibold text-blue-100">Available balance</p><strong className="mt-2 block text-3xl font-bold tracking-tight">{balance}</strong></div><div className="mt-5 grid gap-4"><label className="grid gap-2 text-sm font-bold text-slate-700">Amount (NGN)<input type="number" min="100" step="100" value={amount} onChange={(event) => setAmount(event.target.value)} className="h-12 rounded-lg border border-slate-200 bg-slate-50 px-4 outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-100" placeholder="5,000" /></label><button onClick={fundWallet} className="inline-flex h-12 items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 text-sm font-bold text-white transition hover:bg-blue-700 disabled:bg-slate-300" type="button" disabled={submitting}><Wallet className="h-4 w-4" /> {submitting ? "Opening PocketFi..." : "Fund with PocketFi"}</button>{notice ? <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm font-semibold text-amber-900">{notice}</div> : null}</div></Surface>
